<?php 
function wwp_add_costum_meta_box()
{
	add_meta_box( 'wwp_meta_services',
	              'Services',
	              'wwp_meta_callback',
	              'service_plugins',
	              'normal',
...
function wwp_meta_callback($post){
	wp_nonce_field( basename( __FILE__ ), 'dwwp_jobs_nonce' );
	$dwwp_stored_meta = get_post_meta( $post->ID );
    var_dump($dwwp_stored_meta['job_id']);
	var_dump($dwwp_stored_meta['job_id'][0] );
	?>
...
add_action('add_meta_boxes','wwp_add_costum_meta_box');
